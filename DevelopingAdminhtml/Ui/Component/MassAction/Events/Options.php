...
namespace TrainingBackend\DevelopingAdminhtml\Ui\Component\MassAction\Events;
...
class Options implements \JsonSerializable
{
    ...
    public function jsonSerialize()
    {
        if ($this->options === null) {
            $options = [];
            $events = $this->collectionFactory->create()->addFieldToSelect('*');
            foreach($events as $option){
                $options[$option->getEventId()] = [
                    'label' => $option->getEventName(),
                    'value' => $option->getEventId()
                ];
            }
            $this->prepareData();
