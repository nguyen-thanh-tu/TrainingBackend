...
namespace TrainingBackend\EAV\Ui\Component\MassAction\Merchant;
...
class Options implements \JsonSerializable
{
    ...
    public function jsonSerialize()
    {
        if ($this->options === null) {
            $options = [];
            $merchant = $this->collectionFactory->create()->addFieldToSelect('*');
            foreach($merchant as $option){
                $options[$option->getEntityId()] = [
                    'label' => $option->getName(),
                    'value' => $option->getEntityId()
                ];
            }
            $this->prepareData();
